...
import { accountColorAt, el } from "./dom.ts";
import { attachSearchKeys, searchPalette } from "./search-palette.ts";
...
function drawView(ctx: RenderContext): void {
  ...
  // 検索はどのページの上にも重なるシートなので、ページの外に置く
  if (ctx.state.searchOpen) {
    ctx.root.append(searchPalette(ctx));
  }
}

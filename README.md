...
- **振替記録**: 振替ページで「確認」を押したときの出金口座・入金口座・金額を保存する。
  ...
  (過去に使ったコメントが候補として出る)。パネルの「誤記録なら取り消す」で
  その振替の記録をその場で削除することもできる(削除はR2同期先にも伝わる)。
  銀行サイトはセッションが切れていても実行まで押せてしまい、振替が成立していないのに
  完了表示を出すことがある。セッション切れの案内が見えている間は記録せず、
  完了表示が少しの間残っていることを確かめてから記録する。

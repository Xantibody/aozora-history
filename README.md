...
- **ログイン中の自動取得**: 銀行サイトにログインしていれば、そのタブと同じ
  セッションで銀行サイト自身のAPIに問い合わせ、つかいわけ口座の残高と
  代表口座(普通預金)の入出金明細を裏で取り込む。
  一覧ページを開かずに振替へ直行してもスナップショットが残る。
  問い合わせは最大10分に1回で、ログイン後の画面(`/bank/…`)でのみ行う。
...
- **ダッシュボード**: ツールバーのボタンから開き、以下を表示する。
  ...
  - 残高推移: 全口座合計の折れ線グラフと、口座ごとの残高の履歴
  - 普通口座: 代表口座の入出金明細(振込・給与など)を日ごとにまとめ、
    入金 +・出金 − の符号付きで表示。日計と、入金・出金での絞り込みが使える。
    各明細にもコメントを付けられる
...
R2上のオブジェクトは素のJSON(`{snapshots, transfers, statements, comments, deletions}`)なので、
...
普通口座の入出金明細も同様にCSV(日付・摘要・金額・残高・コメント)で出せる。

既知の制限:

- 振替は「確認」ボタン押下時点で記録するため、
  確認画面でキャンセルした振替も記録されることがある
  (誤記録は振替履歴の×ボタンで削除できる)。
- 普通口座の入出金明細は最新100件までを取り込む。
  それより前の明細は、拡張を入れる前の分と同じく記録されない。

...
- **ログイン中の自動取得**: 銀行サイトにログインしていれば、そのタブと同じ
  セッションで銀行サイト自身のAPIに問い合わせ、つかいわけ口座の残高・
  入出金明細(代表口座とつかいわけ口座)・定額自動振替の設定を裏で取り込む。
  一覧ページを開かずに振替へ直行してもスナップショットが残る。
  問い合わせは最大10分に1回で、ログイン後の画面(`/bank/…`)でのみ行う。
- **記録にない口座間の移動の拾い直し**: つかいわけ口座の残高は、振替ページを
  経由しない理由でも動く(定額自動振替、つかいわけ口座を引落口座に指定した
  自動引落、別端末やスマホアプリからの振替)。これらを「外部入出金」に混ぜると、
  口座間で動いただけの金額が出金と入金の両方に立ち、収支が実際と食い違う。
  同じスナップショット区間で打ち消し合う増減は口座間の移動として1件の振替に
  組み直し、残った分だけを外部入出金として扱う。組み直した振替は「自動」
  (定額自動振替の設定と一致すればその名前)のラベルを付けて記録済みと区別する。
  出金側・入金側が絞れないときは推測せず、差額のまま残す。
...
- **ダッシュボード**: ツールバーのボタンから開き、以下を表示する。
  ...
  - 右上の歯車ボタンから設定画面(R2同期・インポート/エクスポート・デバッグ)を開ける

設定画面の「デバッグ」欄(既定は非表示)では、最後に銀行APIから取り込んだ結果を
APIごとに確認できる。何件取れたか、記録を更新したか、失敗したなら理由と
スタックトレースが出る。取り込みは銀行サイトのタブで裏側で走るため、
銀行側の仕様が変わって静かに取れなくなっても画面には出ない。それに気づくための欄。
「今すぐ取り込む」で、10分の取得間隔を待たずに走らせることもできる。
...
- 振替の「記録」はこの拡張が動いているブラウザでの操作しか残せない。
  拡張を入れる前の振替や、別端末・スマホアプリからの振替は記録にならない
  (誤記録は振替履歴の×ボタンで削除できる)。
  ただし残高スナップショットが両端に残っていれば、上記の拾い直しで
  口座間の移動として復元される。
- 拾い直しは1つのスナップショット区間に1つの移動があることを前提にしている。
  同じ区間で定額自動振替と自動引落が重なるなど、増減が打ち消し合わない場合は
  分離できず、差額のまま外部入出金として残る。
- 入出金明細は口座ごとに最新100件までを取り込む。
  それより前の明細は、拡張を入れる前の分と同じく記録されない。
- つかいわけ口座ごとの明細を取る画面は銀行サイトのメニューからの動線が
  公開されておらず、いつ仕様が変わってもおかしくない。取得した明細の最新残高が
  その口座の残高と一致しない場合は取り込まない(取り込めなければ、外部入出金は
  摘要なしの「外部」表示に戻るだけで、残高と収支の集計は変わらない)。
  取り込めているかは設定画面のデバッグ欄で確認できる。
